...
IDEA_TEXT_DEFAULT=""               # project idea (optional)
...
usage() {
  cat <<USAGE
Usage: $(basename "$0") [-n project_name] [-r /path/to/root] [-s sparc_mode] [-o topology] [-a agents] [-i idea] [-q quiet]
  ...
  -i  Project idea (optional, uses default if not provided)
  -q  Quiet mode (less Flow logs)
Examples:
  $(basename "$0") -n youtube_intel
  $(basename "$0") -n youtube_intel -s architect -o swarm -a 7
  $(basename "$0") -n chat-app -i "A real-time chat application for teams"
USAGE
  exit 1
}
...
IDEA_TEXT="$IDEA_TEXT_DEFAULT"

while getopts ":n:r:s:o:a:i:q" opt; do
  case $opt in
...
# ------------------------- CAPTURE IDEA ----------------------------
# Use provided idea or prompt for input
if [[ -z "$IDEA_TEXT" ]]; then
