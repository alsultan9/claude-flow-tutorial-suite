...
npm list -g claude-flow@2.0.0 >/dev/null 2>&1 || npm i -g claude-flow@2.0.0
...
apply_sparc_mode() {
  ...
  case "$mode" in
    "architect")
      ./claude-flow sparc run architect "design system architecture for project" $NONINTERACTIVE $VERBOSE_FLAG
      ;;
    "api")
      ./claude-flow sparc run api "setup API development environment" $NONINTERACTIVE $VERBOSE_FLAG
      ;;
    "ui")
      ./claude-flow sparc run ui "setup web development environment" $NONINTERACTIVE $VERBOSE_FLAG
      ;;
    "ml")
      ./claude-flow sparc run ml "setup machine learning environment" $NONINTERACTIVE $VERBOSE_FLAG
      ;;
    "tdd")
      ./claude-flow sparc run tdd "setup test-driven development environment" $NONINTERACTIVE $VERBOSE_FLAG
      ;;
    *)
...
echo "  - ./claude-flow sparc run tdd \"Implement feature X end-to-end.\" $NONINTERACTIVE $VERBOSE_FLAG"
